...
###############attempt with internal branches##############

rate_summary_internal <- function(rjpp_out, time_tree, lookup.table, group_column, taxa, min_clade_size=2){
  ...
  time_trees <- lapply(1:length(clades), function(x) keep.tip(time_tree, (filter(GROUP == clades[x]) %>% pull(SPECIES))))
  ...
  #combine into two tables, one for unscaled one for scaled 
