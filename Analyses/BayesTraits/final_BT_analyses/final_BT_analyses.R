...
for (i in 1:length(runs)){
    #keeping pc axes for 90% of variance, multiply by 1000 to make sure tiny values dont underflow in BayesTraits
    scores <- phypc$x[,c(1:which(cumsum(phypc$d/sum(phypc$d))>0.90)[1])]*1000 
    write.table(scores, file = paste0(btfolder,"/Phylo_PC_SCORES_","tree75_80_95","_nasal_", list_of_models[ii],"_",runs[i],"_run",".txt"), quote = FALSE, col.names=FALSE)
    write_cmd_file(x=scores, module_name = paste0("nasal_",runs[i]), model = list_of_models[ii], varrate=TRUE,folder=btfolder)
    }
